...
struct HLD {
    ...
    HLD(vector<vector<int>> &G) {
        n = G.size(); par.resize(n); val.resize(n, 1); vale.resize(n, 1); tin.resize(n);
        tout.resize(n); sz.resize(n); top.resize(n);
        dfs(0, G);
        dfs2(0, G);
        tree_v = segtree(n); tree_e = segtree(n);
        LG = 0; while(1 << LG < n) LG++;
        up.resize(LG + 1, vector<int>(n)); up[0] = par;
    ...
    void dfs(int v, vector<vector<int>> &G, int p = -1) {
        ...
        par[v] = p != -1 ? p : v; sz[v] = 1;
        for (int to : G[v]) {
            dfs(to, G, v); sz[v] += sz[to];
        }
        sort(G[v].begin(), G[v].end(), [&](int x, int y) { return sz[x] > sz[y]; });
    }

    void dfs2(int v, vector<vector<int>> &G) {
        static int timer = 0;
        tin[v] = timer++;
        if (!G[v].empty()) top[G[v][0]] = top[v];
    ...
    int lca(int v, int u) {
        ...
        for (int i = LG; i >= 0; i--) if (!isPar(up[i][v], u)) v = up[i][v];
        return up[0][v];
    }
    ...
    vector<pair<int, int>> get_e_path(int v, int u) {
        ...
        for (auto elem : get_e_vert_path(u, l)) res.push_back(elem);
        return res;
    }
    ...
    void mul_v(int v, int u, int delta) {
        for (auto [l, r] : get_v_path(v, u)) tree_v.update(l, r, delta);
    }

    void mul_e(int v, int u, int delta) {
        for (auto [l, r] : get_e_path(v, u)) tree_e.update(l, r, delta);
    }

    int get_v_sum(int v, int u) {
        ll res = 0; for (auto [l, r] : get_v_path(v, u)) res += tree_v.get_sum(l, r);
        return res % mod;
    }

    int get_e_sum(int v, int u) {
        ll res = 0; for (auto [l, r] : get_e_path(v, u)) res += tree_e.get_sum(l, r);
        return res % mod;
    }
};
